      ...
      <div className="flex justify-center items-center m-auto">
        <div className="fixed top-0 z-50 max-w-xl m-3 px-3 h-10 flex items-center justify-between border-2 border-gray-700 dark:border-gray-600 rounded-full bg-white dark:bg-gray-900 shadow-lg">
          <span className="w-auto hover:scale-105 transition-all duration-300 mr-0 md:mr-10">
            <Link href={createPathWithLang("/", lng)} className="font-semibold dark:text-white flex items-center gap-1 cursor-pointer">
            ...
            <nav className="flex gap-2 md:gap-4">
              {mainNavigation.map((item, index) => (
                <Link
                  key={index}
                  // href={item.href}
                  href={pathname ? createPathWithLang(item.href, lng) : `/${lng}`}
                  className={`relative transition-all duration-300 ${pathname.endsWith(item.href)
